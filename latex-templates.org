...
#+BEGIN_SRC latex
  \RequirePackage{tikz}
  \usetikzlibrary{positioning,arrows,snakes,backgrounds,patterns,matrix,shapes,fit,calc,shadows,plotmarks,spy,external}
#+END_SRC
...
#+BEGIN_SRC latex
  ...
  \NewDocumentEnvironment{cbox}{o o o d()}{
    \begin{center}
      \begin{minipage}[c]{\IfNoValueOrEmptyTF{#1}{0.9\linewidth}{#1}}
        \begin{beamerboxesrounded}[upper=\IfNoValueOrEmptyTF{#2}{lbtuc}{#2}, lower=\IfNoValueOrEmptyTF{#2}{lbtuc}{#2}, shadow=false]
          {\IfNoValueOrEmptyTF{#3}{}{#3}\IfNoValueOrEmptyTF{#4}{}{#4}}
  ...
  \defbeamertemplate*{footnote}{custom}
                     {
                       \parindent 1em\noindent%
                       \raggedright
                       \hbox to 18em{\hfil\insertfootnotemark}\insertfootnotetext%
                     }
#+END_SRC
