...
:PROPERTIES:
:HEADER-ARGS: :tangle memoir-article-style.sty
:END:
...
#+BEGIN_SRC latex
  ...
  \let\origurl\url
  \renewcommand{\url}[1]{{\relsize{-1}\origurl{#1}\,\textcolor{gray}{\scalebox{0.65}{\faExternalLink*}}}}
#+END_SRC
...
#+BEGIN_SRC latex
    \makechapterstyle{article-sans}{
      ...
      \postdate{\end{flushleft}\vskip -10em}
...
#+BEGIN_SRC latex
  ...
  %% \setminted{bgcolor=solarizedbg!80}
...
*** DocDB article
:PROPERTIES:
:HEADER-ARGS: :tangle supernemo-article-style.sty
:END:
     ...
     \ifthenelse{\boolean{snemo@nologo}}{}{
       \resizebox{!}{35mm}{
         \begin{tikzpicture}[y=-1cm]
           ...
           \sffamily\fontsize{30.0}{36.0}\selectfont{}
...
#+BEGIN_SRC latex
  ...
  \sectionfont{\clearemptydoublepage\color{sncolor}}
#+END_SRC
***** SN@ilWare logo
#+BEGIN_SRC latex
...
*** Notes
:PROPERTIES:
:HEADER-ARGS: :tangle supernemo-note-style.sty
:END:
**** Basics
#+BEGIN_SRC latex
  \NeedsTeXFormat{LaTeX2e}
...
#+BEGIN_SRC latex
  \RequirePackage{sectsty}
  \sectionfont{\clearemptydoublepage\color{blue}}
#+END_SRC
***** SN@ilWare logo
#+BEGIN_SRC latex
  \def\Snailware{\mbox{$\overline{\text{SN@il}}$%
...
** Teaching classes
:PROPERTIES:
:HEADER-ARGS: :tangle teaching-classes-style.sty
:END:
*** Basics
#+BEGIN_SRC latex
  \NeedsTeXFormat{LaTeX2e}
  ...
  \renewcommand*{\maketitle}{%
    {
      ...
      \hbox{
        ...
        \parbox[b]{0.8\textwidth}{
          {\noindent\bfseries\sffamily\Huge\@title}\\[2\baselineskip]
          \vspace{0.5\textheight}

          \parbox[b]{0.55\textwidth}{
            \color{gray}
            \small\bfseries\sffamily\@author
            \if\@email\@empty
            \else
...
#+BEGIN_SRC latex
  \RequirePackage{sectsty}
  \sectionfont{\clearemptydoublepage\color{default}}
...
#+BEGIN_SRC latex
  ...
  \newcommand{\codemark}{\raisebox{-1pt}{}}
  \RequirePackage[framemethod=tikz]{mdframed}
  \newmdenv[%
    singleextra={
      \fill[gray] (P) rectangle ([xshift=-15pt]P|-O);
      \node[overlay,anchor=north west,rotate=-90,font=\color{white},yshift=1pt] at (P) {\codemark};
    },
    firstextra={
      \fill[gray] (P) rectangle ([xshift=-15pt]P|-O);
      \node[overlay,anchor=north west,rotate=-90,font=\color{white},yshift=1pt] at (P) {\codemark};
...
#+BEGIN_SRC latex
  %% \newcommand{\questionmark}{\raisebox{-1pt}{\faQuestionCircle}\textbf{\textsf{question}}}
  \newcommand{\questionmark}{\raisebox{-1pt}{}}
  \RequirePackage[framemethod=tikz]{mdframed}
  \newmdenv[%
    singleextra={
      \fill[default] (P) rectangle ([xshift=-15pt]P|-O);
      \node[overlay,anchor=north west,rotate=-90,font=\color{white},yshift=1pt] at (P) {\questionmark};
    },
    firstextra={
      \fill[default] (P) rectangle ([xshift=-15pt]P|-O);
      \node[overlay,anchor=north west,rotate=-90,font=\color{white},yshift=1pt] at (P) {\questionmark};
...
#+BEGIN_SRC latex :results none :tangle no
  ...
  \else
   \@setplength{infocolwidth}{0.1867\paperwidth}%
   \fi
#+END_SRC
...
*** English
:PROPERTIES:
:HEADER-ARGS: :tangle english.lco
:END:
...
#+BEGIN_SRC latex
  ...
  \setkomavar{fromaddress}{Laboratoire de physique des 2 infinis - Irène Joliot-Curie\\Centre Scientifique d'Orsay\\91898 Orsay Cedex}
  \setkomavar{fromemail}{xavier.garrido@ijclab.in2p3.fr}
  \setkomavar{fromphone}{+33 1 64 46 84 28}
  \setkomavar{fromfax}{}
  \setkomavar{fromurl}{}
  %%\setkomavar{fromlogo}{\includegraphics[width=3cm]{logo_upsud_bw}}
  \setkomavar{place}{Orsay}
  \setkomavar{signature}{Xavier Garrido\\Assistant professor at University Paris-Saclay}
#+END_SRC
...
#+BEGIN_SRC latex
  ...
  %%\setkomavar{fromlogo}{\includegraphics[width=3cm]{logo_upsud_bw}}
  \setkomavar{place}{Orsay}
  \setkomavar{signature}{Xavier Garrido\\Maître de conférences à l'Université Paris-Saclay}
#+END_SRC
...
*** Private
:PROPERTIES:
:HEADER-ARGS: :tangle private.lco
:END:
...
#+BEGIN_SRC latex
  \setkomavar{fromname}{Xavier Garrido}
  \setkomavar{fromaddress}{10 bis rue du Lunain\\94230 Cachan}
...
* Beamer styles
:PROPERTIES:
:HEADER-ARGS: :tangle custom-beamer.sty
:END:
...
#+BEGIN_SRC latex :results output :tangle no
  ...
  \setbeamercolor{white}{fg=white, bg=black}
#+END_SRC
...
#+BEGIN_SRC latex
  \ifxetex
  \setmonofont[Scale=0.9]{Inconsolata}
  \newfontfamily{\Handwritten}{Gunny Rewritten}
  \fi
  ...
  \defbeamertemplate*{part page}{custom}[1][colsep=-4bp,
    ...
    \begin{centering}
      ...
      \begin{beamercolorbox}[sep=16pt,center,#1]{part title}
        \usebeamerfont{part title}\bfseries\appendixname\par
      \end{beamercolorbox}
    \end{centering}
  }
#+END_SRC
...
#+BEGIN_SRC latex :noweb yes
  \ifthenelse{\boolean{has_driver_name}}{
    \defbeamertemplate{footline}{cbfootline}{%
      ...
      \hspace{1em}\inserttitle$-$\insertdate\hfill\insertframenumber\hspace{1em}
      \vspace{1pt}
    }{}
  ...
  \renewcommand{\logo}{
    \resizebox{!}{30mm}{
      \begin{tikzpicture}[y=-1cm]
        ...
        \sffamily\bfseries\fontsize{40.0}{36.0}\selectfont{}
...
#+BEGIN_SRC latex
  \renewcommand{\logo}{
    ...
    \definecolor{c094268}{RGB}{9,66,104}
    \definecolor{c615e5e}{RGB}{97,94,94}
    \colorlet{c6ec72e}{gray}
    \colorlet{c094268}{blue}
    \colorlet{c615e5e}{gray}
    %%\tikzsetnextfilename{upsud_logo}
    \resizebox{!}{15mm}{
      \begin{tikzpicture}[y=0.80pt,x=0.80pt,yscale=-1, inner sep=0pt, outer sep=0pt]
        \begin{scope}[cm={{1.25,0.0,0.0,-1.25,(0.0,259.175)}}]
...
#+BEGIN_SRC latex
  ...
  \setbeamertemplate{frametitle}[default][right,rightskip=\the\dimexpr\beamer@rightmargin-0.8cm\relax]
  \ifthenelse{\boolean{cb@noheaderlogo}}{}{
    \addtobeamertemplate{frametitle}{}{%
      \begin{textblock}{14}(0.0,0.5)
        \headerlogo
      \end{textblock}
    }
  }
...
#+BEGIN_SRC latex
  ...
  \let\orighref\href
  \renewcommand{\href}[2]{\orighref{#1}{#2\,\scalebox{0.75}{\faExternalLink*}}}
  %%\renewcommand{\url}[1]{\tikz[overlay]\node[draw=blue,inner sep=1pt, anchor=text, rectangle, rounded corners=1pt, blue] {\smaller[0.75]{\origurl{\texttt{#1}}}};\phantom{#1}}
#+END_SRC
**** Postamble
#+BEGIN_SRC latex :noweb yes
  <<generate_beamer_colors>>
...
* Exporting styles
:PROPERTIES:
:HEADER-ARGS: :tangle latex-templates.sh
:END:
